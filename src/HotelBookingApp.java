import java.util.ArrayList;
import java.util.List;


public class HotelBookingApp {
    public static void main(String[] args) {
        System.out.println("--- Welcome to Book My Stay App ---");

        List<Room> roomInventory = new ArrayList<>();
        roomInventory.add(new Room(101, "Standard"));
        roomInventory.add(new Room(102, "Standard"));
        roomInventory.add(new Room(201, "Deluxe"));
        roomInventory.add(new Room(301, "Suite"));

        // Searching for "Deluxe" rooms
        String searchCategory = "Deluxe";
        System.out.println("\nSearching for category: " + searchCategory);

        boolean found = false;
        for (Room room : roomInventory) {
            if (room.getCategory().equalsIgnoreCase(searchCategory)) {
                System.out.println("Found: " + room);
                found = true;
            }
        }

        if (!found) {
            System.out.println("No rooms found in category: " + searchCategory);
        }
    }
}